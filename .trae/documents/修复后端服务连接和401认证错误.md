...
1. **后端服务状态**：服务正在运行，但PostgreSQL连接失败（connect ECONNREFUSED ::1:27017），服务继续运行但部分功能受限
2. **401错误原因**：
...
### 2. 检查并修复PostgreSQL连接问题

**步骤1：确认PostgreSQL服务状态**
- 检查PostgreSQL是否已安装并运行
- 确认连接字符串配置正确

**步骤2：调整后端配置**
- 如果PostgreSQL不可用，考虑添加友好的错误处理
- 确保服务在没有PostgreSQL的情况下仍能提供核心功能
...
2. **然后检查PostgreSQL连接**：根据实际情况决定是否需要修复
