# 快速开始指南

本指南将帮助您快速上手使用内容解析、管理与热点发现系统的 API。

---

## 📋 目录

1. [环境准备](#环境准备)
2. [启动后端服务](#启动后端服务)
3. [认证流程](#认证流程)
4. [基础接口调用](#基础接口调用)
5. [常见场景示例](#常见场景示例)
6. [故障排查](#故障排查)

---

## 环境准备

### 系统要求

- **Node.js**: >= 14.x
- **PostgreSQL**: >= 12.x
- **Python**: >= 3.8（用于内容解析 SDK）
- **操作系统**: Windows / macOS / Linux

### 依赖安装

#### 1. 克隆项目
```bash
git clone <repository-url>
cd videoAll
```

#### 2. 安装后端依赖
```bash
cd backend
npm install
```

#### 3. 配置环境变量
复制 `.env.example` 为 `.env` 并配置：

```env
# 服务器配置
PORT=3000
NODE_ENV=development

# 数据库配置
DB_HOST=localhost
DB_PORT=5432
DB_NAME=videoall
DB_USER=postgres
DB_PASSWORD=postgres

# JWT 配置
JWT_SECRET=your-secret-key-change-this-in-production
JWT_EXPIRES_IN=7d

# 存储路径
STORAGE_ROOT_PATH=./media

# 日志级别
LOG_LEVEL=info

# 速率限制
RATE_LIMIT_WINDOW_MS=900000
RATE_LIMIT_MAX_REQUESTS=100
```

#### 4. 初始化数据库
```bash
# 确保 PostgreSQL 正在运行
# 创建数据库
createdb videoall

# 或使用 psql
psql -U postgres
CREATE DATABASE videoall;
\q
```

---

## 启动后端服务

### 开发模式（支持热重载）
```bash
cd backend
npm run dev
```

服务将在 `http://localhost:3000` 启动。

### 生产模式
```bash
cd backend
npm run build
npm start
```

### 使用 PM2（推荐用于生产）
```bash
npm install -g pm2
pm2 start backend/src/server.js --name videoall-api
pm2 list
pm2 logs videoall-api
```

### 验证服务运行
```bash
curl http://localhost:3000/api/v1/auth/system-status
```

预期响应：
```json
{
  "hasUsers": false,
  "userCount": 0
}
```

---

## 认证流程

### 1. 系统初始化（仅首次）

如果是全新安装，需要创建第一个管理员账户：

**请求**：
```bash
curl -X POST "http://localhost:3000/api/v1/auth/initial-setup" \
  -H "Content-Type: application/json" \
  -d '{
    "username": "admin",
    "email": "admin@example.com",
    "password": "admin123"
  }'
```

**响应**（201 Created）：
```json
{
  "message": "系统初始化成功",
  "user": {
    "id": "uuid",
    "username": "admin",
    "email": "admin@example.com",
    "role": "admin"
  },
  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
}
```

> **注意**：系统只能初始化一次，后续调用此接口会返回错误。

### 2. 用户登录

**请求**：
```bash
curl -X POST "http://localhost:3000/api/v1/auth/login" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "admin@example.com",
    "password": "admin123"
  }'
```

**响应**（200 OK）：
```json
{
  "message": "登录成功",
  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6InV1aWQiLCJ1c2VybmFtZSI6ImFkbWluIiwicm9sZSI6ImFkbWluIiwiaWF0IjoxNzM1NDQzMDAwLCJleHAiOjE3MzYwNDc4MDB9.signature",
  "user": {
    "id": "uuid",
    "username": "admin",
    "email": "admin@example.com",
    "role": "admin"
  }
}
```

### 3. 使用 Token

在后续需要认证的请求中，在请求头中添加 Token：

```bash
curl -X GET "http://localhost:3000/api/v1/users/me" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
```

**请求头格式**：
```
Authorization: Bearer <your-token-here>
```

### 4. Token 有效期

- **默认有效期**: 7 天
- **过期处理**: 收到 401 状态码时，需重新登录获取新 Token

### 5. 开发环境 Mock Token

开发环境支持使用 Mock Token 进行测试：

```bash
# Mock 管理员 Token
export TOKEN="mock-token-admin"

curl -X GET "http://localhost:3000/api/v1/users/me" \
  -H "Authorization: Bearer $TOKEN"
```

> **警告**：生产环境不支持 Mock Token。

---

## 基础接口调用

### 示例 1：解析内容链接

**小红书内容解析**：

```bash
curl -X POST "http://localhost:3000/api/v1/content/parse" \
  -H "Content-Type: application/json" \
  -d '{
    "link": "https://www.xiaohongshu.com/explore/12345678"
  }'
```

**响应**（201 Created）：
```json
{
  "message": "解析成功",
  "title": "小红书作品标题",
  "author": "作者名称",
  "platform": "xiaohongshu",
  "content_id": "12345678",
  "description": "作品描述...",
  "media_type": "video",
  "cover_url": "https://...",
  "media_url": "https://...",
  "all_images": ["https://...", "https://..."],
  "all_videos": ["https://..."],
  "like_count": 1234,
  "collect_count": 567,
  "comment_count": 89,
  "share_count": 45,
  "view_count": 10000,
  "publish_time": "2025-12-28T10:00:00.000Z",
  "tags": ["标签1", "标签2"],
  "source_url": "https://www.xiaohongshu.com/explore/12345678",
  "source_type": 1,
  "created_at": "2025-12-28T12:00:00.000Z"
}
```

### 示例 2：获取内容列表

```bash
curl -X GET "http://localhost:3000/api/v1/content?page=1&pageSize=20&platform=xiaohongshu"
```

**响应**（200 OK）：
```json
{
  "message": "获取成功",
  "data": [
    {
      "id": "uuid",
      "title": "内容标题",
      "author": "作者",
      "platform": "xiaohongshu",
      ...
    }
  ],
  "pagination": {
    "total": 100,
    "page": 1,
    "pageSize": 20,
    "totalPages": 5
  }
}
```

### 示例 3：保存内容到数据库

```bash
curl -X POST "http://localhost:3000/api/v1/content/save" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "内容标题",
    "author": "作者名称",
    "platform": "xiaohongshu",
    "content_id": "12345678",
    "description": "描述",
    "media_type": "video",
    "cover_url": "https://...",
    "source_url": "https://www.xiaohongshu.com/explore/12345678"
  }'
```

---

## 常见场景示例

### 场景 1：完整的内容解析和下载流程

```bash
# 1. 登录获取 Token
TOKEN=$(curl -s -X POST "http://localhost:3000/api/v1/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email": "admin@example.com", "password": "admin123"}' \
  | jq -r '.token')

# 2. 解析内容链接
curl -X POST "http://localhost:3000/api/v1/content/parse" \
  -H "Content-Type: application/json" \
  -d '{"link": "https://www.xiaohongshu.com/explore/12345678"}'

# 3. 保存到数据库
curl -X POST "http://localhost:3000/api/v1/content/save" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"link": "https://www.xiaohongshu.com/explore/12345678"}'

# 4. 查询已保存的内容
curl -X GET "http://localhost:3000/api/v1/content?page=1&pageSize=10" \
  -H "Authorization: Bearer $TOKEN"
```

### 场景 2：获取仪表盘统计数据

```bash
curl -X GET "http://localhost:3000/api/v1/dashboard/stats" \
  -H "Authorization: Bearer $TOKEN"
```

**响应**：
```json
{
  "message": "获取成功",
  "data": {
    "totalContents": 1000,
    "totalAuthors": 150,
    "totalViews": 500000,
    "todayNewContents": 25,
    "platformDistribution": {
      "xiaohongshu": 600,
      "douyin": 400
    }
  }
}
```

### 场景 3：获取热搜数据

```bash
# 获取所有平台热搜
curl -X GET "http://localhost:3000/api/v1/hotsearch/all" \
  -H "Authorization: Bearer $TOKEN"
```

---

## 故障排查

### 问题 1：无法连接到后端服务

**症状**：
```
curl: (7) Failed to connect to localhost port 3000
```

**解决方案**：
1. 检查后端服务是否正在运行：
   ```bash
   pm2 list
   # 或
   ps aux | grep node
   ```

2. 检查端口是否被占用：
   ```bash
   lsof -i :3000
   ```

3. 查看后端日志：
   ```bash
   tail -f backend/logs/combined.log
   ```

### 问题 2：数据库连接失败

**症状**：
```json
{
  "message": "数据库连接失败"
}
```

**解决方案**：
1. 检查 PostgreSQL 是否运行：
   ```bash
   # macOS
   brew services list

   # Linux
   systemctl status postgresql
   ```

2. 检查数据库配置：
   ```bash
   psql -U postgres -h localhost -p 5432 -d videoall
   ```

3. 验证 `.env` 文件中的数据库配置

### 问题 3：认证失败（401）

**症状**：
```json
{
  "message": "Token无效或已过期"
}
```

**解决方案**：
1. 重新登录获取新 Token
2. 检查 Token 格式：`Authorization: Bearer <token>`
3. 确认 Token 未过期（默认7天）
4. 开发环境可使用 Mock Token 测试

### 问题 4：解析内容失败

**症状**：
```json
{
  "message": "解析失败: 网络连接超时"
}
```

**解决方案**：
1. 检查网络连接
2. 确认链接格式正确
3. 检查平台是否支持（目前仅小红书、抖音）
4. 配置平台 Cookie 提高成功率
5. 查看解析错误日志：
   ```bash
   tail -f backend/logs/parse-error.log
   ```

### 问题 5：速率限制（429）

**症状**：
```json
{
  "status": "error",
  "message": "请求过于频繁，请稍后重试",
  "code": 429
}
```

**解决方案**：
1. 等待 15 分钟后重试
2. 实现请求队列和节流
3. 调整速率限制配置（.env 文件）

---

## 下一步

现在您已经掌握了基础 API 的使用方法，可以：

1. 📖 阅读详细模块文档：
   - [认证模块详解](./modules/01_认证模块.md)
   - [内容管理详解](./modules/02_内容管理.md)
   - [其他模块文档](./README.md#文档导航)

2. 📊 查看数据模型：
   - [Content 实体](./data_models/Content_实体.md)
   - [User 实体](./data_models/User_实体.md)

3. 💡 参考代码示例：
   - [Android (Kotlin)](./examples/Kotlin_Retrofit.md)
   - [React (JavaScript)](./examples/JavaScript_Axios.md)
   - [iOS (Swift)](./examples/Swift_Alamofire.md)

4. 🔧 查看 OpenAPI 规范：
   - [openapi.yaml](./swagger/openapi.yaml)

---

**最后更新**: 2025-12-28
